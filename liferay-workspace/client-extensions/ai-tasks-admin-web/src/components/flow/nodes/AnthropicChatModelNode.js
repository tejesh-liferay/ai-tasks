...
const AnthropicChatModelNode = ({ id, data }) => {
  const { label, parameters } = data;
  const { toggleDropdown } = useNodeMenu();

  return (
    <div className="anthropic-chat-model-node">
      ...
      <div className={'node-header-container'}>
        ...
        <span className="label label-info">
          <span className="label-item label-item-expand">Anthropic Chat Model</span>
        </span>
      </div>
...
export default memo(AnthropicChatModelNode);
